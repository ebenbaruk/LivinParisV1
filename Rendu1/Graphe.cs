    ...
    public class Graphe<T>
    {
        ...
        public (List<int> chemin, double distance) Dijkstra(int depart, int arrivee)
        {
            ...
            while (nonVisites.Count > 0)
            {
                ...
                int u = nonVisites.OrderBy(v => distances[v]).First();
                
                // Si on est arrivé à destination ou si la distance est infinie, on s'arrête
                if (u == arrivee || distances[u] == double.PositiveInfinity)
                    break;
                    
                nonVisites.Remove(u);
                
                var noeudU = Noeuds.FirstOrDefault(n => n.Id == u);
                if (noeudU == null) continue;
                
                // Pour chaque voisin non visité
                foreach (var voisin in noeudU.Voisins)
                {
                    if (!nonVisites.Contains(voisin.Id))
                        continue;
                        
                    // Trouver le lien entre u et v
                    var lien = Liens.FirstOrDefault(l => l.Source.Id == u && l.Destination.Id == voisin.Id);
                    if (lien == null && !EstOriente)
                        lien = Liens.FirstOrDefault(l => l.Source.Id == voisin.Id && l.Destination.Id == u);
                    ...
                    if (precedents[u] != -1)
                    {
                        var noeudPrecedent = Noeuds.FirstOrDefault(n => n.Id == precedents[u]);
                        
                        // Vérifier si les deux noeuds contiennent des objets Station
                        if (noeudPrecedent != null && noeudU.Donnees is Station && noeudPrecedent.Donnees is Station)
                        {
                            var stationU = noeudU.Donnees as Station;
                            var stationPrecedente = noeudPrecedent.Donnees as Station;
                            
                            // Si les deux stations sont sur des lignes différentes, ajouter le temps de correspondance
                            if (stationU != null && stationPrecedente != null && stationU.Ligne != stationPrecedente.Ligne)
                                alt += TempsChangementLigne;
                        }
                    }
                    
                    if (alt < distances[voisin.Id])
                    {
                        distances[voisin.Id] = alt;
                        precedents[voisin.Id] = u;
                    }
                }
            }
